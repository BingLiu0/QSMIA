...
from .build_model import AttackModel, MLP
...
def mia_base(args, config, target_model, dataloaders_target, dataset_sizes_target, shadow_model, dataloaders_shadow, dataset_sizes_shadow):
    data_test_set, label_test_set = get_data_for_mia_base(target_model, dataloaders_target, dataset_sizes_target, args.device)
    data_train_set, label_train_set = get_data_for_mia_base(shadow_model, dataloaders_shadow, dataset_sizes_shadow, args.device)
    
    train_indices = torch.randperm(data_train_set.size(0))
    data_train_set = data_train_set[train_indices]
    label_train_set = label_train_set[train_indices]
    
    test_indices = torch.randperm(data_test_set.size(0))
    data_test_set = data_test_set[test_indices]
    label_test_set = label_test_set[test_indices]
    
    dataset_train_attack = torch.utils.data.TensorDataset(data_train_set, label_train_set)
    ...
    dataset_test_attack = torch.utils.data.TensorDataset(data_test_set, label_test_set)
    ...
    attack_model = MLP(config.general.num_classes).to(args.device)
    optimizer = optim.SGD(attack_model.parameters(), lr=config.attack_learning.learning_rate, momentum=config.attack_learning.momentum, weight_decay=config.attack_learning.weight_decay)
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=config.attack_learning.decrease_lr_factor, gamma=config.attack_learning.decrease_lr_every)
    criterion = nn.CrossEntropyLoss()
